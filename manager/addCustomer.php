...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    {
        global $conn;
        $result = $conn->query("SELECT * FROM customer WHERE $column = '$value' LIMIT 1");
    ...
    function generateRandomUserId() {
        return '201' . str_pad(mt_rand(0, 99999), 5, '0', STR_PAD_LEFT);
    }
    ...
    if (!empty($_FILES["customer-profile-image"]["name"])) {
        $target_file = $target_dir . basename($_FILES["customer-profile-image"]["name"]);
        ...
        if (move_uploaded_file($_FILES["customer-profile-image"]["tmp_name"], $target_file)) {
    ...
    // Escape and sanitize user inputs
    $firstname = sanitizeInput($_POST['customer-firstname']);
    $lastname = sanitizeInput($_POST['customer-lastname']);
    $address = sanitizeInput($_POST['customer-home-address']);
    $contact_number = sanitizeInput($_POST['customer-contact-number']);
    $username = sanitizeInput($_POST['customer-username']);
    $password = $_POST['customer-password'];// Hash the password for security

    $customer_role = "Customer";

    $customer_id = generateRandomUserId();

    // Check if any field is empty
    if (empty($firstname) || empty($lastname) || empty($address) || empty($contact_number) || empty($username) || empty($_POST['customer-password'])) {
        ...
        $sql = "INSERT INTO customer (firstname, lastname, address, con_number, username, password, user_image, user_id, user_role)
                VALUES ('$firstname', '$lastname', '$address', '$contact_number', '$username', '$password', '$image_path', '$customer_id', '$customer_role')";
