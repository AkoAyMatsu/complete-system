<?php
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        // Get all distinct checkout_ids with pending status orders for the specific user
        $getCheckoutIdsQuery = "SELECT DISTINCT co.checkout_id
                                ...
                                WHERE coo.order_status = '$getStatus' AND coo.user_id = '$userId'";
        ...
        if ($result->num_rows > 0) {
            ...
            // Select orders related to the checkout_ids with pending status, ordered by checkout date, for the specific user
            $getOrdersQuery = "SELECT co.*, p.*, coo.*, pm.*, ch.* 
                                FROM " . TABLE_CHECKOUT_ORDER . " co
