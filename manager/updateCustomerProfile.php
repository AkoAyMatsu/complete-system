...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    {
        global $conn;
        $result = $conn->query("SELECT * FROM customer WHERE $column = '$value' AND user_id != '$id' LIMIT 1");
    ...
    $target_dir = "../../bwrs/sample_images/CUSTOMER_"; // Replace with your desired directory
    $default_image_path = "../../bwrs/sample_images/user-icon.png"; // Replace with the path to your default image

    // Initialize image path
    $image_path = '';

    if (!empty($_FILES["edit-customer-profile-image"]["name"])) {
        $target_file = $target_dir . basename($_FILES["edit-customer-profile-image"]["name"]);

        $file_type = $_FILES["edit-customer-profile-image"]["type"];
        ...
        if (move_uploaded_file($_FILES["edit-customer-profile-image"]["tmp_name"], $target_file)) {
    ...
    } elseif (!empty($_POST['customer-image-path'])) {
        // No new image uploaded, use the path from the form
        $image_path = sanitizeInput($_POST['customer-image-path']);
    } else {
    ...
    // Escape and sanitize user inputs
    $firstname = sanitizeInput($_POST['edit-customer-firstname']);
    $lastname = sanitizeInput($_POST['edit-customer-lastname']);
    $address = sanitizeInput($_POST['edit-customer-home-address']);
    $contact_number = sanitizeInput($_POST['edit-customer-contact-number']);
    $username = sanitizeInput($_POST['edit-customer-username']);
    $password = $_POST['edit-customer-password']; // Hash the password for security

    $customer_role = "Customer";

    $customer_id = sanitizeInput($_POST['customer-id']);

    // Check if any field is empty
    if (empty($firstname) || empty($lastname) || empty($address) || empty($contact_number) || empty($username) || empty($_POST['edit-customer-password'])) {
    ...
    } elseif (isDuplicate('username', $username, $customer_id)) {
        // Check if the username already exists in the database
        $response['message'] = 'Username already exists.';
    } elseif (isDuplicate('con_number', $contact_number, $customer_id)) {
       ...
       // Check if the data has been edited
        if (!empty($_FILES["edit-customer-profile-image"]["name"])) {
            ...
            $sql_check_edited = "SELECT * FROM customer WHERE user_id = '$customer_id' AND
        ...
        if ($result_check_edited === FALSE) {
            ...
            $sql_update = "UPDATE customer 
                        ...
                        user_image='$image_path', user_role='$customer_role'
            ...
            if ($conn->query($sql_update) === TRUE) {
                ...
                // Handle the SQL query error
                $response['message'] = 'Error updating customer data: ' . $conn->error;
            }
        } else {
            // No data edited
