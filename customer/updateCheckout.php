...
if (isset($_SESSION['user_id'])) {
    ...
    if ($data && isset($data['payment_id'], $data['payment_type'], $data['payment_total'], $data['payment_date'], $data['checkout_id'], $data['order_ids'])) {
        ...
        foreach ($order_ids as $orderId) {
            $updateCustomerOrderQuery = "UPDATE " . TABLE_CUSTOMER_ORDER . " SET payment_id = ?, order_status = ? WHERE order_id = ? AND user_id = ?";
            $stmt = $conn->prepare($updateCustomerOrderQuery);
            $stmt->bind_param("ssss", $payment_id, $orderStatus, $orderId, $current_user_id);
            $stmt->execute();
            $stmt->close();
        }

        // Update checkout table with payment_id
        $updateCheckoutQuery = "UPDATE " . TABLE_CHECKOUT . " SET payment_id = ? WHERE checkout_id = ? AND user_id = ?";
        $stmt = $conn->prepare($updateCheckoutQuery);
        $stmt->bind_param("sss", $payment_id, $checkout_id, $current_user_id);
        $stmt->execute();
        $stmt->close();

        // Update checkout_order table with payment_id
        $checkoutOrderIds = implode(",", $order_ids);
        $updateCheckoutOrderQuery = "UPDATE " . TABLE_CHECKOUT_ORDER . " SET payment_id = ? WHERE checkout_id = ? AND user_id = ?";
        $stmt = $conn->prepare($updateCheckoutOrderQuery);
        $stmt->bind_param("sss", $payment_id, $checkoutId, $current_user_id);
        $stmt->execute();
        $stmt->close();
